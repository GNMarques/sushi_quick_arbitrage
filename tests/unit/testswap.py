...
def swap_eth_for_erc20(erc20_address, amount, acct):
    ...
    router.swapExactETHForTokens(0, ["0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270",
                                 erc20_address], acct.address, 9999999999999999, {"from": acct, "value": amount})
...
def test_curve_swap(acct, DAI, USDC, flashloan_v2, set_tokens):
    ...
    flashloan_v2.swap_sushiswap(DAI.balanceOf(
        flashloan_v2.address),  {"from": acct})
    assert USDC.balanceOf(flashloan_v2.address) > 0
    assert DAI.balanceOf(flashloan_v2.address) == 0

    flashloan_v2.swap_quickswap(USDC.balanceOf(
        flashloan_v2.address), {"from": acct})
    assert DAI.balanceOf(flashloan_v2.address) > 0
    assert USDC.balanceOf(flashloan_v2.address) == 0
